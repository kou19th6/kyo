...
# =====================================================================
# 📈 HỆ THỐNG CHỨNG KHOÁN v3 — SÀN THẬT: ORDER BOOK, SLIPPAGE, EARNINGS
# =====================================================================

SECTOR_NAMES = {
    "BDS": "🏘️ Bất Động Sản", "HK": "✈️ Hàng Không", "TECH": "💻 Công Nghệ",
    "CRYPTO": "🪙 Tiền Số", "AUTO": "🚗 Ô Tô", "BANK": "🏦 Ngân Hàng", "STEEL": "🔩 Thép",
}

STANDARD_STOCKS = {
    "VIN":  {"name": "Tập Đoàn VIN",     "base": 80000,  "sector": "BDS",    "volatility": 0.03,  "beta": 1.1},
    "FLC":  {"name": "Hàng Không FLC",   "base": 12000,  "sector": "HK",     "volatility": 0.05,  "beta": 1.3},
    "VNZ":  {"name": "Công Nghệ VNZ",    "base": 150000, "sector": "TECH",   "volatility": 0.04,  "beta": 1.2},
    "DOGE": {"name": "Doge Coin",        "base": 5000,   "sector": "CRYPTO", "volatility": 0.15,  "beta": 1.8},
    "BTC":  {"name": "Bitcoin",          "base": 500000, "sector": "CRYPTO", "volatility": 0.10,  "beta": 1.6},
    "AAPL": {"name": "Apple Inc.",       "base": 200000, "sector": "TECH",   "volatility": 0.025, "beta": 0.9},
    "TSLA": {"name": "Tesla",            "base": 180000, "sector": "AUTO",   "volatility": 0.07,  "beta": 1.4},
    "VCB":  {"name": "Vietcombank",      "base": 90000,  "sector": "BANK",   "volatility": 0.02,  "beta": 0.6},
    "TCB":  {"name": "Techcombank",      "base": 55000,  "sector": "BANK",   "volatility": 0.025, "beta": 0.7},
    "HPG":  {"name": "Hòa Phát Group",   "base": 40000,  "sector": "STEEL",  "volatility": 0.035, "beta": 1.0},
}

STOCK_TAX_RATE          = 0.001
STOCK_SPREAD            = 0.0015
MARGIN_BASE_SPREAD      = 0.06       # cộng thêm vào lãi suất nền
MAX_MARGIN_RATIO        = 2.0
CIRCUIT_BREAKER         = 0.07
MAINTENANCE_MARGIN_RATIO= 1.3        # kiểu Reg-T: tài sản < 130% nợ ký quỹ -> margin call
EARNINGS_INTERVAL_HOURS = 4

_session_open_prices = {}
_market_events = []

def get_base_interest_rate():
    """Lãi suất nền thị trường, thay đổi theo ngày (giống lãi suất ngân hàng trung ương)."""
    day_seed = int(datetime.now().strftime("%Y%m%d"))
    rng = random.Random(day_seed + 777)
    return round(rng.uniform(0.03, 0.09), 3)

def get_market_index():
    """Chỉ số thị trường chung (VN-Index style), seed theo giờ hiện tại."""
    seed = int(datetime.now().strftime("%Y%m%d%H")) + 424242
...
def get_sector_index(sector):
    """% thay đổi trung bình của các mã trong ngành so với 1 giờ trước."""
    codes = [c for c, d in STANDARD_STOCKS.items() if d["sector"] == sector]
    if not codes:
        return 0.0
    diffs = []
    for c in codes:
        cur = get_stock_price(c)
        prev = get_stock_price(c, hour_offset=-1)
        if prev > 0:
            diffs.append((cur - prev) / prev * 100)
    return sum(diffs) / len(diffs) if diffs else 0.0

def maybe_run_earnings(comp):
    """Chấm điểm 'báo cáo kết quả kinh doanh' cho công ty đã IPO, ảnh hưởng giá cổ phiếu."""
    now = datetime.now()
    last_str = comp.get("last_earnings", "2000-01-01 00:00:00")
    try:
        last = datetime.strptime(last_str, "%Y-%m-%d %H:%M:%S")
    except Exception:
        last = datetime(2000, 1, 1)
    if now - last < timedelta(hours=EARNINGS_INTERVAL_HOURS):
        return
    snap_treasury = comp.get("_fin_snapshot_treasury", comp.get("treasury", 0))
    snap_rep      = comp.get("_fin_snapshot_rep", comp.get("reputation", 100))
    cur_treasury  = comp.get("treasury", 0)
    cur_rep       = comp.get("reputation", 100)

    treasury_growth = 0.0
    if snap_treasury > 0:
        treasury_growth = (cur_treasury - snap_treasury) / snap_treasury
    rep_growth = (cur_rep - snap_rep) / 100.0

    drift = max(-0.06, min(0.06, treasury_growth * 0.4 + rep_growth * 0.3))
    comp["earnings_drift"] = drift
    comp["last_earnings"] = now.strftime("%Y-%m-%d %H:%M:%S")
    comp["_fin_snapshot_treasury"] = cur_treasury
    comp["_fin_snapshot_rep"] = cur_rep
    save_company(comp["_id"])
...
def get_stock_price(stock_code, hour_offset=0, use_cache=True):
    ...
    if ipo_comp:
        COMPANY_CACHE[ipo_comp["_id"]] = ipo_comp
        maybe_run_earnings(ipo_comp)
        ...
        drift = 1 + ipo_comp.get("earnings_drift", 0.0)
        ...
        return max(1000, int(base_price * rep_mult * scandal * drift * fluctuation))
    ...
    dt = 1 / 252
    periods = seed_15min % 26
    market_drift = get_market_index() * beta
    ...
    for i in range(periods):
        ...
        z = r.gauss(0, 1)
        sector_drift = (get_sector_trend(sector) + market_drift) / 26
...
def check_circuit_breaker(code):
    ...
    if open_price == 0:
        return False
    return abs(current - open_price) / open_price >= CIRCUIT_BREAKER

def get_market_depth(code):
    """Độ sâu thanh khoản — mã vốn hóa lớn/ít biến động thì 'chịu' được lệnh lớn hơn."""
    stock_info = STANDARD_STOCKS.get(code)
    if stock_info:
        base = stock_info["base"]
        vol = stock_info["volatility"]
        return max(300, int(80_000_000 / (base * vol * 60)))
    comp = None
    try:
        comp = companies_col.find_one({"is_ipo": True, "name": {"$regex": f"^{code}", "$options": "i"}})
    except Exception:
        pass
    treasury = comp.get("treasury", 1_000_000) if comp else 1_000_000
    return max(150, int(treasury / 80_000))

def get_slippage_pct(code, qty):
    depth = get_market_depth(code)
    impact = qty / depth
    return min(0.30, impact * 0.18)

def get_stock_buy_price(code, qty=1):
    """Giá mua = giá TT + spread + trượt giá theo khối lượng (mua nhiều đắt hơn)."""
    market = get_stock_price(code)
    slip = get_slippage_pct(code, qty)
    return int(market * (1 + STOCK_SPREAD) * (1 + slip))

def get_stock_sell_price(code, qty=1):
    """Giá bán = giá TT - spread - trượt giá - thuế."""
    market = get_stock_price(code)
    slip = get_slippage_pct(code, qty)
    after_impact = market * (1 - STOCK_SPREAD) * (1 - slip)
    tax = int(after_impact * STOCK_TAX_RATE)
    return max(1, int(after_impact) - tax)
...
def get_all_stocks():
    ...
    try:
        for comp in companies_col.find({"is_ipo": True}):
...
async def check_stop_loss_take_profit(user_id, channel):
    ...
    for code, qty in dict(user_data.get("stocks", {})).items():
        if qty <= 0: continue
        current_price = get_stock_sell_price(code, qty)
    ...
    if margin > 0:
        total_stock_value = sum(get_stock_sell_price(c, q) * q for c, q in user_data.get("stocks", {}).items())
        total_assets = user_data.get("money", 0) + user_data.get("bank", 0) + total_stock_value
        if total_assets < margin * MAINTENANCE_MARGIN_RATIO and total_assets > 0:
            for code in list(user_data.get("stocks", {}).keys()):
                if user_data.get("margin_used", 0) <= 0: break
                qty = user_data["stocks"].get(code, 0)
                sp = get_stock_sell_price(code, qty)
                proceeds = sp * qty
    ...
    if sold_msg:
        save_user(user_id)
        try:
            embed = discord.Embed(title="🤖 TỰ ĐỘNG KHỚP LỆNH", description="\n".join(sold_msg), color=discord.Color.orange())
...
async def chungkhoan(ctx):
    ...
    embed = discord.Embed(
        ...
        description=(
            ...
            f"🏦 Lãi suất nền hôm nay: **{base_rate*100:.1f}%/năm** (ảnh hưởng lãi vay ký quỹ)\n"
            f"⚡ Cầu dao: dừng nếu giá biến động >7%/phiên\n\n"
            f"**LỆNH:** `buy` `sell` `order` `orders` `cancel` `sl` `tp` `margin` `short` `covershort` `port` `chart` `market` `ipo`"
        ),
    ...
    for sec, name in SECTOR_NAMES.items():
        ...
        sector_lines.append(f"{icon} {name}: **{idx:+.2f}%**")
    embed.add_field(name="📊 Chỉ Số Ngành", value="\n".join(sector_lines), inline=False)

    for code, name in list(all_stocks.items())[:8]:
        current = get_stock_price(code)
        change_pct = get_price_change_pct(code)
        circuit = "🔴DỪNG" if check_circuit_breaker(code) else ""
        trend = "🟢" if change_pct > 0 else "🔴" if change_pct < 0 else "⚪"
        buy_p = get_stock_buy_price(code, 100)
        sell_p = get_stock_sell_price(code, 100)
        embed.add_field(
...
async def market(ctx):
    """Xem tổng quan chỉ số ngành."""
    embed = discord.Embed(title="📊 CHỈ SỐ THỊ TRƯỜNG", color=discord.Color.teal())
    for sec, name in SECTOR_NAMES.items():
        idx = get_sector_index(sec)
        icon = "🟢" if idx > 0 else "🔴" if idx < 0 else "⚪"
        codes = [c for c, d in STANDARD_STOCKS.items() if d["sector"] == sec]
        embed.add_field(name=f"{icon} {name}", value=f"**{idx:+.2f}%**\nMã: {', '.join(codes)}", inline=True)
    embed.set_footer(text=f"Lãi suất nền: {get_base_interest_rate()*100:.1f}%/năm")
    await ctx.reply(embed=embed, mention_author=False)


@chungkhoan.command()
async def depth(ctx, code: str):
    """Xem độ sâu thanh khoản của 1 mã — càng sâu càng ít bị trượt giá khi đặt lệnh lớn."""
    code = code.upper()
    if code not in get_all_stocks():
        return await ctx.reply("⚠️ Mã CK không tồn tại!")
    d = get_market_depth(code)
    embed = discord.Embed(title=f"📊 ĐỘ SÂU THANH KHOẢN {code}", color=discord.Color.blue())
    for qty in [10, 100, 1000, 5000]:
        slip = get_slippage_pct(code, qty)
        embed.add_field(name=f"Lệnh {qty:,} CP", value=f"Trượt giá: **{slip*100:.2f}%**", inline=True)
    embed.set_footer(text=f"Thanh khoản cơ sở: ~{d:,} CP")
    await ctx.reply(embed=embed, mention_author=False)
...
async def buy(ctx, code: str, qty: int):
    ...
    try:
        now = datetime.now()
        if user_id in stock_cooldowns and (now - stock_cooldowns[user_id]).total_seconds() < 600:
            diff = (now - stock_cooldowns[user_id]).total_seconds()
            return await ctx.reply(embed=discord.Embed(description=f"⏳ Cooldown: còn **{int((600-diff)//60)}p {int((600-diff)%60)}s**", color=discord.Color.orange()), mention_author=False)
        ...
        if check_circuit_breaker(code):
            return await ctx.reply(embed=discord.Embed(description=f"🔴 **CẦU DAO!** Mã {code} tạm ngừng do biến động >7%!", color=discord.Color.red()), mention_author=False)

        buy_price = get_stock_buy_price(code, qty)
        if get_stock_price(code) <= 100:
        ...
        user_data.setdefault("stocks", {})[code] = new_qty
        user_data.setdefault("stock_buy_prices", {})[code] = new_avg
...
async def sell(ctx, code: str, qty_str: str):
    ...
    try:
        now = datetime.now()
        if user_id in stock_cooldowns and (now - stock_cooldowns[user_id]).total_seconds() < 120:
            diff = (now - stock_cooldowns[user_id]).total_seconds()
            return await ctx.reply(embed=discord.Embed(description=f"⏳ Cooldown: còn **{int((120-diff)//60)}p {int((120-diff)%60)}s**", color=discord.Color.orange()), mention_author=False)

        code = code.upper()
        user_data = load_user(user_id)
        owned = user_data.get("stocks", {}).get(code, 0)
        qty = owned if qty_str.lower() == "all" else int(qty_str)
        if qty <= 0: return await ctx.reply("⚠️ Số lượng > 0!")
        if owned < qty: return await ctx.reply(embed=discord.Embed(description=f"⚠️ Chỉ có **{owned:,} CP {code}**!", color=discord.Color.red()), mention_author=False)
        if check_circuit_breaker(code):
            return await ctx.reply(embed=discord.Embed(description=f"🔴 **CẦU DAO!** Mã {code} tạm ngừng!", color=discord.Color.red()), mention_author=False)

        sell_price = get_stock_sell_price(code, qty)
...
async def order(ctx, code: str, side: str, qty: int, price: int):
    user_id = str(ctx.author.id)
    code = code.upper(); side = side.lower()
    if side not in ["buy", "sell", "mua", "ban", "bán"]:
        return await ctx.reply("⚠️ Dùng: `k ck order <MÃ> buy/sell <SL> <GIÁ>`")
    if code not in get_all_stocks(): return await ctx.reply("⚠️ Mã không tồn tại!")
    if qty <= 0 or qty > 10000: return await ctx.reply("⚠️ SL 1-10000!")
    if price <= 0: return await ctx.reply("⚠️ Giá > 0!")
    ...
    if is_buy:
        ...
        if owned < qty: return await ctx.reply(f"⚠️ Chỉ có {owned} CP, không đủ để đặt bán {qty} CP!")

    pending_order = {"user_id": user_id, "code": code, "side": "buy" if is_buy else "sell", "qty": qty, "price": price,
                      "created": datetime.now().strftime("%Y-%m-%d %H:%M:%S"), "status": "pending"}
    user_data.setdefault("stock_orders", [])
    ...
    embed = discord.Embed(
        ...
        description=(f"Mã: **{code}** | SL: **{qty:,} CP** | Giá đặt: **{price:,}**\nGiá TT: **{current:,}**\n\n"
...
async def list_orders(ctx):
    ...
    for o in orders[:25]:
        ...
        embed.add_field(name=f"#{o['id']} {side_emoji} {o['side'].upper()} {o['code']}",
                         value=f"SL: {o['qty']:,} | Giá đặt: **{o['price']:,}** | Hiện tại: **{current:,}**", inline=False)
    await ctx.reply(embed=embed, mention_author=False)
...
async def cancel(ctx, order_id: int):
    user_id = str(ctx.author.id)
    user_data = load_user(user_id)
    target = next((o for o in user_data.get("stock_orders", []) if o.get("id") == order_id and o.get("status") == "pending"), None)
    if not target: return await ctx.reply("⚠️ Không tìm thấy lệnh này!")
...
async def sl(ctx, code: str, price: int):
    ...
    code = code.upper()
    user_data = load_user(user_id)
    if code not in user_data.get("stocks", {}): return await ctx.reply(f"⚠️ Bạn chưa có cổ phiếu {code}!")
    current = get_stock_price(code)
    if price >= current: return await ctx.reply(f"⚠️ Giá SL phải thấp hơn giá hiện tại ({current:,})!")
    user_data.setdefault("stop_loss", {})[code] = price
    save_user(user_id)
    await ctx.reply(embed=discord.Embed(title="🛑 ĐẶT STOP-LOSS", description=f"Mã: **{code}** | Giá SL: **{price:,}**", color=discord.Color.red()), mention_author=False)
...
async def tp(ctx, code: str, price: int):
    ...
    if code not in user_data.get("stocks", {}): return await ctx.reply(f"⚠️ Bạn chưa có cổ phiếu {code}!")
    current = get_stock_price(code)
    if price <= current: return await ctx.reply(f"⚠️ Giá TP phải cao hơn giá hiện tại ({current:,})!")
    user_data.setdefault("take_profit", {})[code] = price
    save_user(user_id)
    await ctx.reply(embed=discord.Embed(title="✅ ĐẶT TAKE-PROFIT", description=f"Mã: **{code}** | Giá TP: **{price:,}**", color=discord.Color.green()), mention_author=False)
...
async def margin(ctx, code: str, qty: int):
    ...
    try:
        code = code.upper()
        if code not in get_all_stocks(): return await ctx.reply("⚠️ Mã không tồn tại!")
        if qty <= 0: return await ctx.reply("⚠️ SL > 0!")
        ...
        stock_value = sum(get_stock_sell_price(c, q) * q for c, q in user_data.get("stocks", {}).items())
        ...
        daily_interest = int(cash_needed * rate / 365)
        save_user(user_id)
...
async def short(ctx, code: str, qty: int):
    user_id = str(ctx.author.id)
    if not acquire_lock(user_id): return await ctx.reply("⚠️ Đang xử lý!", mention_author=False)
    try:
        code = code.upper()
        if code not in STANDARD_STOCKS: return await ctx.reply("⚠️ Chỉ bán khống cổ phiếu tiêu chuẩn!")
        if qty <= 0 or qty > 1000: return await ctx.reply("⚠️ SL 1-1000 CP!")
        user_data = load_user(user_id)
        ...
        user_data["short_positions"][code] = user_data["short_positions"].get(code, 0) + qty
        user_data["short_buy_prices"][code] = market_price
        save_user(user_id)
...
async def covershort(ctx, code: str, qty: int):
    ...
    try:
        ...
        if short_qty <= 0: return await ctx.reply(f"⚠️ Không có vị thế bán khống {code}!")
        if qty > short_qty: return await ctx.reply(f"⚠️ Chỉ có {short_qty} CP!")
        short_price = user_data.get("short_buy_prices", {}).get(code, 0)
        current_price = get_stock_price(code)
        total_profit = (short_price - current_price) * qty
        collateral_return = short_price * qty
...
async def port(ctx, member: discord.Member = None):
    ...
    if stocks:
        ...
        for code, qty in stocks.items():
            ...
            avg_buy = user_data.get("stock_buy_prices", {}).get(code, sell_p)
            val = sell_p * qty
    ...
    if shorts:
        ...
        for code, qty in shorts.items():
            ...
            pnl = (entry - curr) * qty
            lines.append(f"**{code}** SHORT {qty:,}CP | Entry: {entry:,} | P&L: **{pnl:+,}**")
...
async def chart(ctx, code: str):
    code = code.upper()
    if code not in get_all_stocks(): return await ctx.reply("⚠️ Mã không tồn tại!")
    prices = [get_stock_price(code, hour_offset=h) for h in range(-5, 1)]
    min_p, max_p = min(prices), max(prices)
    price_range = max_p - min_p if max_p != min_p else 1
    chart_lines = []
    for row in range(6, -1, -1):
        threshold = min_p + (price_range * row / 6)
        line = "".join("█" if p >= threshold else "░" for p in prices)
        chart_lines.append(f"`{threshold:>10,.0f}` {line}")
...
async def ipo(ctx):
    ...
    comp["_fin_snapshot_rep"] = comp.get("reputation", 100)
    comp["earnings_drift"] = 0.0
    save_company(comp_id)
    code = comp["name"][:4].upper()
    await ctx.reply(embed=discord.Embed(title="📈 CHÀO SÀN THÀNH CÔNG",
        description=f"**{comp['name']}** đã IPO!\nMã: **{code}** | Giá cổ phiếu sẽ dao động theo hiệu quả kinh doanh thật của công ty (báo cáo mỗi {EARNINGS_INTERVAL_HOURS}h)",
        color=discord.Color.green()), mention_author=False)


# =====================================================================
# BACKGROUND TASKS — giữ nguyên tên hàm để chỗ khởi động task không đổi
# =====================================================================
        ...
        try:
            ...
            for doc in all_users:
                ...
                for order in user_data.get("stock_orders", []):
                    ...
                    should_fill = (is_buy and current_price <= order["price"]) or (not is_buy and current_price >= order["price"])
                    if should_fill:
                        order["status"] = "filled"; changed = True
                        qty = order["qty"]
                        if is_buy:
                            old_qty = user_data["stocks"].get(code, 0)
        ...
        try:
            ...
            for doc in all_users:
                ...
                if now - last >= timedelta(days=1):
                    daily_interest = int(margin * rate / 365)
        ...
        try:
            ...
            for doc in all_users:
                ...
                for code in list(user_data.get("stocks", {}).keys()):
                    qty = user_data["stocks"].get(code, 0)
                    if qty <= 0: continue
                    current = get_stock_sell_price(code, qty)
...
# =====================================================================
# =====================================================================
# 🏢 HỆ THỐNG CÔNG TY v2 — PHÒNG BAN SINH LỜI, TÍN DỤNG, BCTC THẬT
# =====================================================================

DEPARTMENTS_INFO = {
    "sales":     {"name": "🛒 Kinh Doanh", "base_cost": 300_000, "income_per_lv": 8_000},
    "rd":        {"name": "🔬 R&D",        "base_cost": 500_000, "income_per_lv": 5_000, "rep_bonus": 1},
    "marketing": {"name": "📢 Marketing",  "base_cost": 250_000, "income_per_lv": 4_000, "rep_bonus": 2},
    "security":  {"name": "🛡️ An Ninh",    "base_cost": 400_000, "income_per_lv": 0},  # giảm rủi ro gián điệp/bóc phốt
}
DEPARTMENT_TICK_HOURS = 1

def get_departments(comp):
    return comp.setdefault("departments", {k: 0 for k in DEPARTMENTS_INFO})

def get_credit_rating(comp):
    """Xếp hạng tín dụng AAA -> D dựa trên danh tiếng và nợ/quỹ."""
    rep = comp.get("reputation", 100)
    debt = comp.get("debt", 0)
    treasury = max(1, comp.get("treasury", 0))
    debt_ratio = debt / treasury
    score = rep - debt_ratio * 60
    if score >= 90:  return "AAA", 0.00
    if score >= 75:  return "AA",  0.02
    if score >= 60:  return "A",   0.05
    if score >= 45:  return "BBB", 0.09
    if score >= 30:  return "BB",  0.15
    if score >= 15:  return "B",   0.25
    return "D", 0.40

def compute_company_financials(comp):
    """Doanh thu/chi phí/lợi nhuận thật của công ty theo phòng ban + nợ."""
    deps = get_departments(comp)
    revenue = sum(deps.get(k, 0) * v["income_per_lv"] for k, v in DEPARTMENTS_INFO.items())
    debt = comp.get("debt", 0)
    _, spread = get_credit_rating(comp)
    daily_interest_expense = int(debt * (get_base_interest_rate() + spread) / 365)
    member_count = len(comp.get("members", {}))
    payroll_expense = member_count * 1000  # chi phí vận hành cơ bản/giờ mô phỏng
    expenses = daily_interest_expense // 24 + payroll_expense
    net_income = revenue - expenses
    return {"revenue": revenue, "expenses": expenses, "net_income": net_income}

async def process_department_income():
    """Phòng ban tự sinh doanh thu mỗi giờ — CẦN được start ở nơi bot khởi động các task khác."""
    await bot.wait_until_ready()
    while not bot.is_closed():
        try:
            all_comps = list(companies_col.find({"departments": {"$exists": True}}))
            for doc in all_comps:
                comp_id = doc["_id"]
                COMPANY_CACHE[comp_id] = doc
                comp = COMPANY_CACHE[comp_id]
                fin = compute_company_financials(comp)
                comp["treasury"] = comp.get("treasury", 0) + max(0, fin["net_income"])
                if fin["net_income"] < 0:
                    comp["treasury"] = max(0, comp["treasury"] + fin["net_income"])
                save_company(comp_id)
        except Exception as e:
            print(f"[WARN] process_department_income error: {e}")
        await asyncio.sleep(DEPARTMENT_TICK_HOURS * 3600)
...
async def cty(ctx):
    ...
    cmds = (
        ...
        "**Phòng ban:** `k cty phongban` — xây phòng ban sinh lời thụ động\n"
        "**Tài chính:** `k cty baocaotaichinh` | `k cty codividend` — chia cổ tức thật từ lợi nhuận\n"
...
async def tao(ctx, *, name: str):
    ...
    new_comp = {
        ...
        "departments": {k: 0 for k in DEPARTMENTS_INFO},
    }
    COMPANY_CACHE[user_id] = new_comp; save_company(user_id); save_user(user_id)
    await ctx.send(embed=discord.Embed(title="🏢 KHAI TRƯƠNG", description=f"Chúc mừng {ctx.author.mention} đã thành lập **{name}**!\n`k cty` để mở bảng điều khiển.", color=discord.Color.green()))


@cty.command()
async def phongban(ctx, dept: str = None, levels: int = 1):
    """Nâng cấp phòng ban để sinh doanh thu thụ động mỗi giờ."""
    user_id = str(ctx.author.id); comp_id = load_user(user_id).get("company")
    if not comp_id: return await ctx.reply("Bạn chưa có công ty!", mention_author=False)
    comp = load_company(comp_id)
    if comp["members"].get(user_id) not in ["boss", "quanly"]: return await ctx.reply("Chỉ Ban Giám Đốc mới quản lý phòng ban!", mention_author=False)

    deps = get_departments(comp)
    if not dept:
        fin = compute_company_financials(comp)
        embed = discord.Embed(title=f"🏭 PHÒNG BAN — {comp['name']}", color=discord.Color.teal())
    ...
    dept = dept.lower()
    if dept not in DEPARTMENTS_INFO: return await ctx.reply(f"⚠️ Phòng ban gồm: {', '.join(DEPARTMENTS_INFO.keys())}", mention_author=False)
    if levels < 1 or levels > 50: return await ctx.reply("⚠️ Nâng tối đa 50 cấp/lần!", mention_author=False)
...
@cty.command()
async def baocaotaichinh(ctx):
    """Xem báo cáo tài chính thật của công ty."""
    user_id = str(ctx.author.id); comp_id = load_user(user_id).get("company")
    if not comp_id: return await ctx.reply("Bạn chưa có công ty!", mention_author=False)
    comp = load_company(comp_id)
    fin = compute_company_financials(comp)
...
@cty.command()
async def codividend(ctx):
    """Chia cổ tức THẬT dựa trên lợi nhuận ròng, trả cho toàn bộ cổ đông đang nắm giữ mã CP công ty."""
    user_id = str(ctx.author.id); comp_id = load_user(user_id).get("company")
    if not comp_id: return await ctx.reply("Bạn chưa có công ty!", mention_author=False)
    comp = load_company(comp_id)
    if comp["members"].get(user_id) != "boss": return await ctx.reply("Chỉ Chủ Tịch mới chia cổ tức!", mention_author=False)
    if not comp.get("is_ipo"): return await ctx.reply("⚠️ Công ty chưa IPO, chưa có cổ đông ngoài!", mention_author=False)

    fin = compute_company_financials(comp)
    if fin["net_income"] <= 0:
        return await ctx.reply("⚠️ Công ty đang lỗ, không có lợi nhuận để chia cổ tức!", mention_author=False)

    code = comp["name"][:4].upper()
    payout_pool = int(fin["net_income"] * 24 * 0.5)  # 50% lợi nhuận 1 ngày đem chia
    if comp["treasury"] < payout_pool:
        payout_pool = comp["treasury"]
    if payout_pool <= 0:
        return await ctx.reply("⚠️ Quỹ không đủ để chia cổ tức!", mention_author=False)

    try:
        holders = list(users_col.find({f"stocks.{code}": {"$gt": 0}}))
    except Exception:
        holders = []
    if not holders:
        return await ctx.reply("⚠️ Chưa có ai nắm giữ cổ phiếu công ty bạn!", mention_author=False)

    total_shares = sum(h.get("stocks", {}).get(code, 0) for h in holders)
    if total_shares <= 0:
        return await ctx.reply("⚠️ Không có cổ phần nào đang lưu hành!", mention_author=False)

    per_share = payout_pool / total_shares
    comp["treasury"] -= payout_pool
    save_company(comp_id)

    paid_count = 0
    for h in holders:
        hid = h["_id"]
        qty = h.get("stocks", {}).get(code, 0)
        amount = int(qty * per_share)
...
@cty.command()
async def sapnhap(ctx, *, target_name: str):
    """Đề nghị sáp nhập công ty khác, giá tính theo định giá vốn hóa thật."""
    user_id = str(ctx.author.id); comp_id = load_user(user_id).get("company")
    if not comp_id: return await ctx.reply("Bạn chưa có công ty!", mention_author=False)
    comp = load_company(comp_id)
    if comp["members"].get(user_id) != "boss": return await ctx.reply("Chỉ Chủ Tịch mới đề nghị sáp nhập!", mention_author=False)

    try:
        all_comps = list(companies_col.find())
    except Exception:
        all_comps = []
    target = next((c for c in all_comps if target_name.lower() in c["name"].lower() and c["_id"] != comp_id), None)
    if not target: return await ctx.reply("⚠️ Không tìm thấy công ty đối thủ!", mention_author=False)
    ...
    await ctx.send(f"<@{target_boss_id}>", embed=embed, view=view)
...
async def tuyen(ctx, member: discord.Member):
    ...
    comp = load_company(comp_id)
    if comp["members"].get(user_id) not in ["boss", "quanly"]: return await ctx.reply("Chỉ GĐ/CT mới được tuyển!", mention_author=False)
...
async def thulai(ctx):
    ...
    now = datetime.now()
    try: last = datetime.strptime(comp.get("last_interest", "2000-01-01 00:00:00"), "%Y-%m-%d %H:%M:%S")
    except Exception: last = datetime(2000, 1, 1)
...
async def quangcao(ctx, amount: int):
    ...
    if not comp_id: return await ctx.reply("⚠️ Bạn chưa có công ty!", mention_author=False)
    comp = load_company(comp_id)
    if comp["members"].get(user_id) not in ["boss", "quanly"]: return await ctx.reply("⚠️ Chỉ Ban Giám Đốc mới chạy quảng cáo!", mention_author=False)
    now = datetime.now()
    ...
    if random.uniform(0, 100) <= 12:
        ...
        result = f"💀 **PHẢN CẢM!** Danh tiếng **{rep_change}**!"; color = discord.Color.red()
    else:
...
async def boicong(ctx, *, target_name: str):
    ...
    if not comp_id: return await ctx.reply("⚠️ Bạn chưa có công ty!", mention_author=False)
    comp = load_company(comp_id)
    if comp["members"].get(user_id) != "boss": return await ctx.reply("⚠️ Chỉ Chủ Tịch!", mention_author=False)
    now = datetime.now()
    ...
    if not target or target["_id"] == comp_id: return await ctx.reply("⚠️ Không tìm thấy!", mention_author=False)
    COMPANY_CACHE[target["_id"]] = target
    comp["treasury"] -= cost; comp["last_smear"] = now.strftime("%Y-%m-%d %H:%M:%S")
    success_chance = max(20, 65 - target.get("security_level", 1) * 5)
    if random.uniform(0, 100) <= success_chance:
        ...
        if target["reputation"] <= 30 and not target.get("has_scandal"): target["has_scandal"] = True
        save_company(target["_id"]); save_company(comp_id)
        result = f"🕵️ **THÀNH CÔNG!** **{target['name']}** mất **{rep_loss}** danh tiếng!"; color = discord.Color.dark_red()
    else:
        comp["reputation"] = max(0, comp.get("reputation", 100) - 25); comp["has_scandal"] = True
        save_company(comp_id)
        result = "🚨 **BỊ PHÁT HIỆN!** Tự dính scandal, -25 danh tiếng!"; color = discord.Color.red()
    await ctx.reply(embed=discord.Embed(title="🕵️ BÓC PHỐT", description=result, color=color), mention_author=False)
...
async def giandiep(ctx, *, target_name: str):
    ...
    if not comp_id: return await ctx.reply("⚠️ Bạn chưa có công ty!", mention_author=False)
    comp = load_company(comp_id)
    if comp["members"].get(user_id) not in ["boss", "quanly"]: return await ctx.reply("⚠️ Chỉ Ban Giám Đốc!", mention_author=False)
    now = datetime.now()
    ...
    if not target or target["_id"] == comp_id: return await ctx.reply("⚠️ Không tìm thấy!", mention_author=False)
    COMPANY_CACHE[target["_id"]] = target
    comp["treasury"] -= cost; comp["last_spy"] = now.strftime("%Y-%m-%d %H:%M:%S")
    success_chance = max(15, 55 - target.get("security_level", 1) * 6)
    if random.uniform(0, 100) <= success_chance:
...
async def cotuc(ctx, percent: int):
    """Chia thưởng nhanh cho nội bộ (khác với codividend là chia cho cổ đông ngoài sàn)."""
    user_id = str(ctx.author.id); comp_id = load_user(user_id).get("company")
    if not comp_id: return await ctx.reply("⚠️ Bạn chưa có công ty!", mention_author=False)
    comp = load_company(comp_id)
    if comp["members"].get(user_id) != "boss": return await ctx.reply("⚠️ Chỉ Chủ Tịch!", mention_author=False)
    if percent <= 0 or percent > 50: return await ctx.reply("⚠️ Chia 1-50%!", mention_author=False)
    now = datetime.now()
    try: last = datetime.strptime(comp.get("last_dividend", "2000-01-01 00:00:00"), "%Y-%m-%d %H:%M:%S")
    except Exception: last = datetime(2000, 1, 1)
    if now - last < timedelta(days=1): return await ctx.reply(f"⏳ 1 lần/ngày! <t:{int((last+timedelta(days=1)).timestamp())}:R>", mention_author=False)
    fin = compute_company_financials(comp)
    base_for_split = max(0, fin["net_income"]) * 24 if fin["net_income"] > 0 else comp.get("treasury", 0)
    total_pay = int(min(comp.get("treasury", 0), base_for_split) * percent / 100)
    if total_pay <= 0: return await ctx.reply("⚠️ Không có gì để chia (công ty chưa có lợi nhuận)!", mention_author=False)
    members = comp.get("members", {})
    each = total_pay // max(1, len(members))
    comp["treasury"] -= total_pay; comp["last_dividend"] = now.strftime("%Y-%m-%d %H:%M:%S")
    for m_id in members:
...
async def chucvu(ctx, member: discord.Member, role: str):
    ...
    if not comp_id: return
    comp = load_company(comp_id)
    if comp["members"].get(user_id) != "boss": return await ctx.reply("Chỉ Chủ tịch!", mention_author=False)
    target_id = str(member.id)
    if target_id not in comp["members"]: return await ctx.reply("Người này không thuộc công ty.", mention_author=False)
    if target_id == user_id: return await ctx.reply("Không thể tự đổi chức!", mention_author=False)
    if role not in ["quanly", "nhanvien"]: return await ctx.reply("Chức vụ: `quanly` hoặc `nhanvien`.", mention_author=False)
    comp["members"][target_id] = role; save_company(comp_id)
    await ctx.reply(f"✅ Đặt chức {member.mention} thành **{comp['roles'][role]}**.", mention_author=False)
...
async def roi(ctx):
    ...
    if my_role == "boss":
        ...
        embed_bankrupt = discord.Embed(description="🏢 Chủ tịch bỏ trốn! Công ty **PHÁ SẢN**!", color=discord.Color.red())
...
async def moctui(ctx, member: discord.Member):
   ...
   # ── KIỂM TRA VÍ CHỐNG MÓC ────────────────────────────────────────
    ...
    await msg.edit(embed=embed)
@bot.command(aliases=['jail', 'banggiam', 'batnguoi'])
async def batgiam(ctx, member: discord.Member, minutes: int, *, reason: str = "Vi phạm nội quy server"):
    """Admin bắt giam người chơi. Cần quyền Administrator hoặc Admin Cấp Cao."""
    if not (ctx.author.guild_permissions.administrator or is_super_admin_id(ctx.author.id) or ctx.author.id in OWNER_IDS):
        return await ctx.reply("⛔ Bạn không có quyền bắt giam người khác!", mention_author=False)

    if member.bot:
        return await ctx.reply("⚠️ Không thể giam bot!", mention_author=False)
    if minutes <= 0 or minutes > 10080:
        return await ctx.reply("⚠️ Thời gian giam từ 1 phút đến 10,080 phút (7 ngày)!", mention_author=False)

    target_id = str(member.id)
    target_data = load_user(target_id)
    jail_end = datetime.now() + timedelta(minutes=minutes)
    target_data["jail_time"] = jail_end.strftime("%Y-%m-%d %H:%M:%S")
    target_data["court_used"] = False
    target_data["crime_record"] = target_data.get("crime_record", 0) + 1
    save_user(target_id)
    add_history(target_id, f"Bị Admin {ctx.author.name} bắt giam {minutes}p (Lý do: {reason})")

    prison_data, prison_lvl = get_prison_data(ctx.guild.id)

    embed = discord.Embed(
        title="🚨 LỆNH BẮT GIAM CỦA ADMIN!",
        description=(
            f"{member.mention} đã bị **{ctx.author.mention}** ra lệnh bắt giam!\n\n"
            f"📜 **Lý do:** {reason}\n"
    ...
    embed.set_image(url=GIF_LINKS.get("jail", ""))
    await ctx.send(embed=embed)

    try:
        await member.send(embed=discord.Embed(
            description=f"🚨 Bạn vừa bị Admin **{ctx.author.name}** bắt giam **{minutes} phút** tại **{ctx.guild.name}**!\nLý do: {reason}",
            color=discord.Color.red()
        ))
    except Exception:
        pass
...
@bot.command(aliases=['thagiam', 'unjail'])
async def thagiamnguoi(ctx, member: discord.Member):
    """Admin thả người đang bị giam."""
    if not (ctx.author.guild_permissions.administrator or is_super_admin_id(ctx.author.id) or ctx.author.id in OWNER_IDS):
        return await ctx.reply("⛔ Không có quyền!", mention_author=False)
    target_data = load_user(member.id)
    if not target_data.get("jail_time"):
        return await ctx.reply(f"⚠️ {member.name} không đang bị giam!", mention_author=False)
    target_data["jail_time"] = None
    save_user(member.id)
    await ctx.reply(embed=discord.Embed(description=f"✅ Đã thả {member.mention} khỏi tù!", color=discord.Color.green()), mention_author=False)
# =====================================================================
...
def get_prison_data(guild_id):
    ...
    lvl = cfg.get("prison_level", 1)
    lvl = max(1, min(lvl, MAX_PRISON_LEVEL))
    return PRISON_LEVELS[lvl], lvl


@bot.command(aliases=['upgradejail', 'nangnhatu'])
async def nangcapnhatu(ctx):
    """Nâng cấp nhà tù server — giam giữ chặt hơn, khó vượt ngục hơn. Cần quyền Admin."""
    if not (ctx.author.guild_permissions.administrator or is_super_admin_id(ctx.author.id) or ctx.author.id in OWNER_IDS):
        return await ctx.reply("⛔ Chỉ Admin mới được nâng cấp nhà tù!", mention_author=False)

    server_id = str(ctx.guild.id)
    cfg = load_server_config(ctx.guild.id)
    cur_lvl = cfg.get("prison_level", 1)

    if cur_lvl >= MAX_PRISON_LEVEL:
        return await ctx.reply(embed=discord.Embed(
            title=f"🏰 {PRISON_LEVELS[cur_lvl]['name']}",
            description="✅ Nhà tù server đã đạt cấp độ TỐI ĐA!",
            color=discord.Color.gold()
        ), mention_author=False)

    next_lvl = cur_lvl + 1
    cost = PRISON_LEVELS[next_lvl]["upgrade_cost"]
    user_data = load_user(ctx.author.id)
    ...
    config_col.update_one({"_id": server_id}, {"$set": {"prison_level": next_lvl}}, upsert=True)
    if server_id not in CONFIG_CACHE:
        CONFIG_CACHE[server_id] = {}
    CONFIG_CACHE[server_id]["prison_level"] = next_lvl

    p = PRISON_LEVELS[next_lvl]
    embed = discord.Embed(
        title="🏗️ NÂNG CẤP NHÀ TÙ THÀNH CÔNG!",
        description=(
...
@bot.command(aliases=['jailinfo', 'nhatu'])
...
class BigCourtTrialView(discord.ui.View):
    def __init__(self, author, plea, prosecutor, stage_index=0, total_score=0):
    ...
    def _make_callback(self, idx):
        async def callback(interaction: discord.Interaction):
            ...
            if self.stage_index + 1 < len(TRIAL_STAGE_POOL):
                ...
                self._build_buttons()
                embed = discord.Embed(
                    title=f"⚖️ PHIÊN TÒA — Giai đoạn {self.stage_index+1}/{len(TRIAL_STAGE_POOL)}",
                    description=(
                        self.log[-1] + f"\n\n{'═'*24}\n"
                        f"**{self.current_stage['stage_title']}**\n{self.current_stage['question']}"
                    ),
                    color=discord.Color.blue()
                )
    ...
    async def _finalize(self, interaction, uid, user_data, jail_end):
        self.stop()
        for c in self.children:
            c.disabled = True

        remain = int((jail_end - datetime.now()).total_seconds())
        ...
        embed.set_footer(text=f"Công tố viên: {prosecutor['name']} | Biện hộ ban đầu: {plea['label']}")
        await interaction.response.edit_message(embed=embed, view=self)


class PleaSelectView(discord.ui.View):
    def __init__(self, author):
        super().__init__(timeout=60)
        self.author = author
        for p in PLEA_OPTIONS:
            btn = discord.ui.Button(label=p["label"][:80], style=discord.ButtonStyle.primary)
            btn.callback = self._make_cb(p)
            self.add_item(btn)

    def _make_cb(self, plea):
        async def cb(interaction: discord.Interaction):
            ...
            if not end or datetime.now() >= end:
                self.stop()
                return await interaction.response.edit_message(
                    embed=discord.Embed(description="✅ Bạn đã được tự do.", color=discord.Color.green()),
                    view=None
                )

            self.stop()
            prosecutor = random.choice(COURT_PROSECUTORS)
            first_stage = TRIAL_STAGE_POOL[0][0]

            embed = discord.Embed(
                title="⚖️ PHIÊN TÒA XÉT XỬ BẮT ĐẦU",
                description=(
            ...
            view = BigCourtTrialView(self.author, plea, prosecutor, total_score=plea.get("base_score", 0))
...
async def toaan(ctx):
    ...
    prison_data, prison_lvl = get_prison_data(ctx.guild.id)
    embed = discord.Embed(
        title=f"⚖️ TRIỆU TẬP HẦU TÒA — {prison_data['name']}",
        description=(
            f"{random.choice(JAIL_CELL_FLAVOR)}\n\n"
            f"Bạn bị áp giải vào phòng xử án uy nghiêm. Trước khi phiên tòa bắt đầu, "
            f"chủ tọa hỏi:\n\n**\"Bị cáo, ngươi biện hộ như thế nào cho tội danh của mình?\"**"
        ),
        color=discord.Color.blue()
    )
    await ctx.reply(embed=embed, view=PleaSelectView(ctx.author), mention_author=False)
CHANNEL_MUTE_SECONDS = 600

async def apply_channel_mute(member: discord.Member, channel, seconds: int):
    try:
        overwrite = channel.overwrites_for(member)
        original_send = overwrite.send_messages
        overwrite.send_messages = False
        await channel.set_permissions(member, overwrite=overwrite, reason="Biệt giam - cấm chat tại kênh vượt ngục")
    except Exception as e:
        print(f"[MUTE] Lỗi áp overwrite: {e}")
        return

    async def _restore():
        await asyncio.sleep(seconds)
        try:
            ov = channel.overwrites_for(member)
            ov.send_messages = original_send
            if ov.is_empty():
                await channel.set_permissions(member, overwrite=None, reason="Hết biệt giam kênh")
            else:
                await channel.set_permissions(member, overwrite=ov, reason="Hết biệt giam kênh - khôi phục")
        except Exception as e:
...
async def vuotngu(ctx):
    ...
    if random.randint(1, 100) <= effective_rate:
        ...
        if user_data["escape_fails"] >= MAX_ESCAPE_FAIL_STREAK:
            ...
            mute_seconds = min(CHANNEL_MUTE_SECONDS, solitary_seconds)
            await apply_channel_mute(ctx.author, ctx.channel, mute_seconds)

            solitary_note = (
                f"\n\n🔒 **BIỆT GIAM!** Cai ngục hết kiên nhẫn với bạn.\n"
                f"Không thể vượt ngục/lao động trong **{sh}h{sm}p** tới!\n"
                f"🔇 Bị **cấm chat tại kênh này** trong **{mute_seconds//60}p{mute_seconds%60:02d}s** "
                f"(các kênh khác vẫn dùng bình thường)."
            )
...
# =====================================================================
# 🎨 HỆ THỐNG QUẢN LÝ ASSET (GIF / EMOJI) - TỰ ĐỘNG PHÁT HIỆN
# Admin cấp cao chỉnh sửa qua bảng nút bấm, không cần sửa code
# =====================================================================
import re

SUPER_ADMIN_IDS = OWNER_IDS + [1377196723998556271]   # 👉 thêm ID admin cấp cao khác vào đây nếu cần

bot_assets_col = db["bot_assets"]
...
DEFAULT_EMOJIS = {
    ...
    "company": "🏢", "stock_up": "📈", "stock_down": "📉",
}
CUSTOM_EMOJIS = dict(DEFAULT_EMOJIS)


# ── HÀM WRAPPER: dùng E("key") thay cho emoji viết cứng ─────────────
def E(key, default_emoji="❓"):
    """Lấy emoji tùy biến. Tự đăng ký key mới ngay lần đầu được gọi."""
    if key in CUSTOM_EMOJIS:
        return CUSTOM_EMOJIS[key]
    val = DEFAULT_EMOJIS.get(key, default_emoji)
    CUSTOM_EMOJIS[key] = val
    DEFAULT_EMOJIS.setdefault(key, val)
    try:
        bot_assets_col.update_one({"_id": "config"}, {"$set": {f"emojis.{key}": val}}, upsert=True)
    except Exception:
        pass
    return val
...
def load_bot_assets():
    ...
    try:
        ...
        print(f"[WARN] load_bot_assets error: {e}")
        doc = None
    if not doc:
        doc = {"_id": "config", "gifs": {}, "emojis": {}}
        try: bot_assets_col.insert_one(doc)
        except Exception: pass
    for k, v in doc.get("gifs", {}).items():
        GIF_LINKS[k] = v
...
def save_gif(key, url):
    GIF_LINKS[key] = url
    try:
        bot_assets_col.update_one({"_id": "config"}, {"$set": {f"gifs.{key}": url}}, upsert=True)
    except Exception as e:
        print(f"[WARN] save_gif error: {e}")


def save_emoji(key, emoji):
    CUSTOM_EMOJIS[key] = emoji
    try:
        bot_assets_col.update_one({"_id": "config"}, {"$set": {f"emojis.{key}": emoji}}, upsert=True)
    except Exception as e:
        print(f"[WARN] save_emoji error: {e}")


# ── QUÉT FILE NGUỒN ĐỂ TỰ PHÁT HIỆN ASSET MỚI ────────────────────────
def scan_source_for_assets():
    """Đọc lại chính file bot.py, tìm GIF_LINKS[...] và E(...) chưa được đăng ký."""
    try:
        with open(BOT_FILE_PATH, "r", encoding="utf-8") as f:
            source = f.read()
    except Exception as e:
        print(f"[SCAN] Lỗi đọc file: {e}")
        return [], []
    ...
    # --- GIF ---
    gif_patterns = [
        r'GIF_LINKS\[\s*["\']([\w\-]+)["\']\s*\]\s*=\s*["\']([^"\']+)["\']',
        r'GIF_LINKS\.get\(\s*["\']([\w\-]+)["\']\s*,\s*["\']([^"\']*)["\']\s*\)',
        r'GIF_LINKS\[\s*["\']([\w\-]+)["\']\s*\]',
    ]
    found = {}
    for pat in gif_patterns:
        for m in re.finditer(pat, source):
            key = m.group(1)
            val = m.group(2) if m.lastindex and m.lastindex >= 2 else ""
            if key not in found or val:
                found[key] = val
    for key, val in found.items():
        if key not in GIF_LINKS:
            GIF_LINKS[key] = val or "https://media.giphy.com/media/xUPGcguWZHRC2HyBRS/giphy.gif"
            DEFAULT_GIF_LINKS[key] = GIF_LINKS[key]
            new_gifs.append(key)
    ...
    for pat in emoji_patterns:
        for m in re.finditer(pat, source):
            key = m.group(1)
            val = m.group(2) if m.lastindex and m.lastindex >= 2 else ""
            if key not in found_e or val:
                found_e[key] = val
    for key, val in found_e.items():
        if key not in CUSTOM_EMOJIS:
            CUSTOM_EMOJIS[key] = val or "❓"
            DEFAULT_EMOJIS[key] = CUSTOM_EMOJIS[key]
            new_emojis.append(key)

    if new_gifs or new_emojis:
        updates = {}
        for k in new_gifs:
            updates[f"gifs.{k}"] = GIF_LINKS[k]
        for k in new_emojis:
            updates[f"emojis.{k}"] = CUSTOM_EMOJIS[k]
        try:
            bot_assets_col.update_one({"_id": "config"}, {"$set": updates}, upsert=True)
        except Exception as e:
            print(f"[SCAN] Lỗi lưu: {e}")

    return new_gifs, new_emojis


def asset_stats_embed():
    gif_total  = len(GIF_LINKS)
    gif_custom = sum(1 for k, v in GIF_LINKS.items() if DEFAULT_GIF_LINKS.get(k) != v)
    emo_total  = len(CUSTOM_EMOJIS)
    emo_custom = sum(1 for k, v in CUSTOM_EMOJIS.items() if DEFAULT_EMOJIS.get(k) != v)

    embed = discord.Embed(title="🎨 BẢNG ĐIỀU KHIỂN ASSET", color=discord.Color.gold())
    embed.description = (
        f"🎬 GIF: `{make_progress_bar(gif_custom, gif_total, 10)}` **{gif_custom}/{gif_total}** đã chỉnh\n"
        f"😀 Emoji: `{make_progress_bar(emo_custom, emo_total, 10)}` **{emo_custom}/{emo_total}** đã chỉnh\n"
        f"───────────────\n"
        f"👇 Chọn mục bên dưới để chỉnh sửa"
    )
    embed.set_footer(text="Chỉ Admin Cấp Cao mới thao tác được")
    return embed


# ── MODAL NHẬP DỮ LIỆU ──────────────────────────────────────────────
class GifUrlModal(discord.ui.Modal, title="Chỉnh sửa GIF"):
    def __init__(self, key, parent_view):
        super().__init__()
        self.key = key
        self.parent_view = parent_view
        self.url_input = discord.ui.TextInput(
            label=f"URL GIF mới cho '{key}'"[:45],
            placeholder="https://media.giphy.com/media/.../giphy.gif",
            default=GIF_LINKS.get(key, ""),
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        save_gif(self.key, url)
        embed = discord.Embed(title=f"✅ Đã cập nhật GIF `{self.key}`", color=discord.Color.green())
        embed.set_image(url=url)
        await interaction.response.edit_message(embed=embed, view=self.parent_view)


class EmojiValueModal(discord.ui.Modal, title="Chỉnh sửa Emoji"):
    def __init__(self, key, parent_view):
        super().__init__()
        self.key = key
        self.parent_view = parent_view
    ...
    async def on_submit(self, interaction: discord.Interaction):
        ...
        save_emoji(self.key, val)
        embed = discord.Embed(
            title="✅ Đã cập nhật Emoji",
            description=f"`{self.key}` → {val}",
            color=discord.Color.green()
        )
        await interaction.response.edit_message(embed=embed, view=self.parent_view)
...
class AssetMainView(TimeoutSafeView):
    ...
    async def btn_list(self, interaction, button):
        ...
        embed.add_field(name="🎬 GIF", value=(gif_lines[:1000] or "Trống"), inline=False)
        embed.add_field(name="😀 Emoji", value=(emo_lines[:1000] or "Trống"), inline=False)
        await interaction.response.edit_message(embed=embed, view=self)

    @discord.ui.button(label="🔍 Quét Tính Năng Mới", style=discord.ButtonStyle.success, row=1)
    async def btn_scan(self, interaction, button):
        if interaction.user.id != self.author.id:
            return await interaction.response.send_message("Không phải bạn!", ephemeral=True)
        new_g, new_e = scan_source_for_assets()
        if not new_g and not new_e:
            desc = "✅ Không có asset mới nào chưa được đăng ký."
        else:
            desc = ""
            if new_g: desc += f"🎬 GIF mới phát hiện: {', '.join(f'`{k}`' for k in new_g)}\n"
            if new_e: desc += f"😀 Emoji mới phát hiện: {', '.join(f'`{k}`' for k in new_e)}\n"
            desc += "\n👉 Vào **Quản Lý GIF/Emoji** để chỉnh sửa ngay!"
        await interaction.response.edit_message(
            embed=discord.Embed(title="🔍 KẾT QUẢ QUÉT FILE BOT", description=desc, color=discord.Color.blue()),
            view=self
        )

    @discord.ui.button(label="🔄 Reset Toàn Bộ", style=discord.ButtonStyle.danger, row=1)
    async def btn_reset_all(self, interaction, button):
        if interaction.user.id != self.author.id:
            return await interaction.response.send_message("Không phải bạn!", ephemeral=True)
        GIF_LINKS.clear(); GIF_LINKS.update(DEFAULT_GIF_LINKS)
        CUSTOM_EMOJIS.clear(); CUSTOM_EMOJIS.update(DEFAULT_EMOJIS)
        try:
            bot_assets_col.update_one({"_id": "config"}, {"$set": {"gifs": {}, "emojis": {}}}, upsert=True)
        except Exception: pass
        await interaction.response.edit_message(
            embed=discord.Embed(description="🔄 Đã reset toàn bộ GIF & Emoji về mặc định!", color=discord.Color.gold()),
            view=self
        )

    @discord.ui.button(label="❌ Đóng", style=discord.ButtonStyle.secondary, row=1)
    async def btn_close(self, interaction, button):
        ...
        for c in self.children:
            c.disabled = True
        await interaction.response.edit_message(
            embed=discord.Embed(description="✅ Đã đóng bảng điều khiển.", color=discord.Color.dark_grey()),
            view=self
        )
        self.stop()
...
@bot.command(aliases=['asset', 'quanlyasset'])
...
# =====================================================================
# ⚠️ FILE NÀY THAY THẾ HOÀN TOÀN "bridge_control_system.py" trước đó.
#    Nếu đã dán bản cũ vào bot.py, hãy XÓA toàn bộ khối code cũ đó
#    (từ dòng "HỆ THỐNG BRIDGE ĐA SERVER" tới hết các lệnh lienket/
#    huylienket/ds_lienket cũ) rồi dán file này vào thay thế.
#
# CÁCH CÀI ĐẶT:
# 1) Dán TOÀN BỘ file này vào bot.py, TRƯỚC dòng keep_alive()
# 2) Trong on_message hiện tại, đảm bảo có 2 dòng đầu tiên (giữ nguyên
#    như trước, không đổi gì thêm):
#
#        handled = await handle_bridge_relay(message)
#        if handled:
#            return
#
# LƯU Ý DB: Dùng collection mới "bridge_links_v2" (khác tên với bản cũ
# "bridge_links") để không bị lẫn dữ liệu cũ sai định dạng.
#
# ĐIỂM MỚI SO VỚI BẢN TRƯỚC:
# • 1 kênh bridge giờ có thể nhận tin từ NHIỀU kênh đích khác nhau
#   (gộp nhiều server/kênh về chung 1 chỗ để dễ theo dõi).
# • `k huylienket` dùng được từ BẤT KỲ ĐÂU, không cần đứng trong bridge.
...
bridge_links_col   = db["bridge_links_v2"]   # nhiều doc có thể cùng bridge_channel_id
bridge_msg_map_col = db["bridge_msg_map"]    # {_id: relay_message_id, bridge_channel_id, target_channel_id, target_message_id}
bridge_webhook_cache = {}  # {channel_id: discord.Webhook}
...
# ── HELPER: TRUY VẤN LIÊN KẾT ──────────────────────────────────────────
def get_links_for_bridge(bridge_channel_id):
    """Trả về TẤT CẢ liên kết (có thể nhiều) đổ vào 1 kênh bridge."""
    try:
        return list(bridge_links_col.find({"bridge_channel_id": bridge_channel_id}))
    except Exception as e:
        print(f"[BRIDGE] DB lỗi: {e}")
        return []


def get_link_for_target(target_channel_id):
    """1 kênh đích chỉ được thuộc về ĐÚNG 1 bridge — trả về doc hoặc None."""
    try:
        return bridge_links_col.find_one({"target_channel_id": target_channel_id})
    except Exception as e:
        print(f"[BRIDGE] DB lỗi: {e}")
        return None


def create_bridge_link(bridge_channel_id, target_channel, owner_id):
    guild_name = target_channel.guild.name if target_channel.guild else "DM"
    doc_id = f"{bridge_channel_id}_{target_channel.id}"
    bridge_links_col.update_one(
        {"_id": doc_id},
        {"$set": {
...
async def handle_bridge_relay(message: discord.Message) -> bool:
    """Gọi ở đầu on_message. True = đã xử lý xong, on_message nên return.
    False = không liên quan hoặc chỉ quan sát, on_message tiếp tục bình thường."""
    if message.author.bot:
        return False
    if not message.guild:
        return False

    target_link = get_link_for_target(message.channel.id)
    if target_link:
        return await _relay_target_to_bridge(message, target_link)

    bridge_links = get_links_for_bridge(message.channel.id)
    if bridge_links:
        return await _relay_bridge_to_target(message, bridge_links)

    return False


async def _relay_target_to_bridge(message, link):
    """Tin nhắn ở KÊNH ĐÍCH -> đẩy vào kênh bridge (không chặn xử lý XP/lệnh gốc)."""
    bridge_channel = bot.get_channel(link["bridge_channel_id"])
    ...
    # Ghi rõ nguồn gốc trong tên hiển thị -> tránh nhầm lẫn khi nhiều kênh đổ về 1 bridge
    username = f"{message.author.display_name} • #{message.channel.name}"[:80]
    ...
    try:
        if wh:
            ...
            sent = await bridge_channel.send(f"📡 **{username}**: {reply_note}{content}")
    except Exception as e:
    ...
    if sent:
        bridge_msg_map_col.update_one(
            {"_id": sent.id},
            {"$set": {
                "bridge_channel_id": link["bridge_channel_id"],
                "target_channel_id": message.channel.id,
                "target_message_id": message.id,
            }},
            upsert=True
        )
    return False  # kênh gốc vẫn xử lý XP/lệnh bình thường


async def _relay_bridge_to_target(message, bridge_links):
    """Tin nhắn ở KÊNH BRIDGE (owner gõ) -> gửi sang đúng kênh đích."""
    ...
    if message.reference and message.reference.message_id:
        ...
        if mapped:
            ...
            reply_target_channel_id = mapped["target_channel_id"]

    if reply_target_channel_id:
        target_channel = bot.get_channel(reply_target_channel_id)
    elif len(bridge_links) == 1:
        # Bridge chỉ có 1 nguồn duy nhất -> không cần reply cũng biết gửi đi đâu
        target_channel = bot.get_channel(bridge_links[0]["target_channel_id"])
    else:
        # Nhiều nguồn cùng đổ về đây mà không reply -> KHÔNG đoán bừa, cảnh báo rõ
        names = ", ".join(f"#{l['target_channel_name']}" for l in bridge_links)
        await message.channel.send(embed=discord.Embed(
            description=(
                f"⚠️ Kênh này đang gộp **{len(bridge_links)}** nguồn ({names}).\n"
                f"👉 Hãy **Reply** vào đúng tin nhắn cần trả lời để bot biết gửi đi đâu!"
            ),
            color=discord.Color.orange()
        ), delete_after=8)
    ...
    if not target_channel:
        await message.channel.send(embed=discord.Embed(description="⚠️ Không tìm thấy kênh đích! Liên kết có thể đã hỏng.", color=discord.Color.red()))
        return True
...
# ── LỆNH: TẠO / GỘP LIÊN KẾT ────────────────────────────────────────────
@bot.command(aliases=['bridge', 'ketnoi'])
async def lienket(ctx, channel_id: int, bridge_channel_id: int = None):
    """
    Liên kết 1 kênh vào hệ thống bridge. Chỉ Owner.
    - k lienket <channel_id>                    -> tạo kênh bridge MỚI
    - k lienket <channel_id> <bridge_channel_id> -> GỘP vào 1 bridge đã có sẵn
    """
    if ctx.author.id not in OWNER_IDS:
        return await ctx.reply("⛔ Bạn không có quyền dùng lệnh này!", mention_author=False)
    ...
    existing = get_link_for_target(channel_id)
    if existing:
        old_bridge = bot.get_channel(existing["bridge_channel_id"])
        return await ctx.reply(embed=discord.Embed(
            description=(
                f"⚠️ Kênh này đã liên kết tại {old_bridge.mention if old_bridge else '(kênh bridge đã bị xóa)'}!\n"
                f"Dùng `k huylienket {channel_id}` để gỡ trước nếu muốn liên kết lại."
            ),
            color=discord.Color.orange()
        ), mention_author=False)
    ...
    if bridge_channel_id:
        ...
        if not bridge_channel or bridge_channel.guild.id != ctx.guild.id:
            return await ctx.reply(embed=discord.Embed(description="⚠️ Không tìm thấy kênh bridge đó trong server này!", color=discord.Color.red()), mention_author=False)
    else:
        ...
        try:
            bridge_channel = await ctx.guild.create_text_channel(
                name=safe_name, category=category, topic="🔗 Kênh cầu nối đa nguồn"
            )
        except discord.Forbidden:
            return await ctx.reply("⚠️ Bot thiếu quyền tạo kênh ở server này!", mention_author=False)

    create_bridge_link(bridge_channel.id, target_channel, ctx.author.id)
    total_links = len(get_links_for_bridge(bridge_channel.id))
    guild_name  = target_channel.guild.name if target_channel.guild else "DM"

    note = (
        "💬 Chỉ có 1 nguồn duy nhất → gõ chat bình thường tại bridge là gửi thẳng sang.\n"
        if total_links == 1 else
        f"⚠️ Bridge này hiện có **{total_links} nguồn** → PHẢI **Reply** vào đúng tin nhắn "
        "cần trả lời, nếu không bot sẽ không biết gửi đi đâu!\n"
    )
    ...
    embed = discord.Embed(
        ...
        description=(
            ...
            f"🎯 Kênh đích mới: **#{target_channel.name}** ({guild_name})\n"
            f"🔗 Tổng số nguồn đang gộp vào bridge này: **{total_links}**\n\n"
            f"{note}"
            f"🗑️ Gỡ liên kết này (từ bất kỳ đâu): `k huylienket {target_channel.id}`"
        ),
        color=discord.Color.green()
    )
    await ctx.reply(embed=embed, mention_author=False)
...
# ── LỆNH: HỦY LIÊN KẾT (DÙNG ĐƯỢC TỪ BẤT KỲ ĐÂU) ───────────────────────
@bot.command(aliases=['unbridge', 'gohlienket'])
async def huylienket(ctx, channel_id: int = None, mode: str = None):
    """
    Hủy liên kết — dùng được từ BẤT KỲ ĐÂU, không cần đứng trong kênh bridge. Chỉ Owner.
    - k huylienket                     -> hủy liên kết tại kênh đang đứng
    - k huylienket <id>                -> hủy liên kết của kênh đích/bridge đó
    - k huylienket <bridge_id> all     -> hủy TOÀN BỘ liên kết đổ vào bridge đó
    """
    if ctx.author.id not in OWNER_IDS:
        return await ctx.reply("⛔ Không có quyền!", mention_author=False)
    ...
    # ── Hủy TOÀN BỘ liên kết của 1 bridge ──
    if mode and mode.lower() == "all":
        links = get_links_for_bridge(cid)
        if not links:
            return await ctx.reply("⚠️ Không có liên kết nào tại kênh bridge này!", mention_author=False)
        for l in links:
            bridge_msg_map_col.delete_many({"target_channel_id": l["target_channel_id"]})
        bridge_links_col.delete_many({"bridge_channel_id": cid})
        bridge_webhook_cache.pop(cid, None)
        return await ctx.reply(embed=discord.Embed(
            description=f"✅ Đã hủy toàn bộ **{len(links)}** liên kết đổ vào <#{cid}>.",
            color=discord.Color.orange()
        ), mention_author=False)

    # ── Nếu cid là 1 kênh ĐÍCH đang được liên kết -> gỡ đúng liên kết đó ──
    target_link = get_link_for_target(cid)
    if target_link:
        bridge_links_col.delete_one({"_id": target_link["_id"]})
        bridge_msg_map_col.delete_many({"target_channel_id": cid})
        return await ctx.reply(embed=discord.Embed(
            description=f"✅ Đã gỡ liên kết **#{target_link['target_channel_name']}** khỏi <#{target_link['bridge_channel_id']}>.",
            color=discord.Color.green()
        ), mention_author=False)

    # ── Nếu cid là kênh BRIDGE ──
    bridge_links = get_links_for_bridge(cid)
    if bridge_links:
        if len(bridge_links) == 1:
            l = bridge_links[0]
            bridge_links_col.delete_one({"_id": l["_id"]})
            bridge_msg_map_col.delete_many({"target_channel_id": l["target_channel_id"]})
            return await ctx.reply(embed=discord.Embed(
                description=f"✅ Đã gỡ liên kết duy nhất (**#{l['target_channel_name']}**) khỏi <#{cid}>.",
                color=discord.Color.green()
            ), mention_author=False)
        else:
            names = "\n".join(f"• **#{l['target_channel_name']}** — ID: `{l['target_channel_id']}`" for l in bridge_links)
            return await ctx.reply(embed=discord.Embed(
                title=f"⚠️ Kênh này có {len(bridge_links)} liên kết",
                description=(
                    f"{names}\n\n"
                    f"👉 Gỡ 1 kênh cụ thể: `k huylienket <ID kênh đó>`\n"
                    f"👉 Gỡ TẤT CẢ: `k huylienket {cid} all`"
                ),
                color=discord.Color.orange()
            ), mention_author=False)

    return await ctx.reply(embed=discord.Embed(description="⚠️ Không tìm thấy liên kết nào tại đây!", color=discord.Color.red()), mention_author=False)


# ── LỆNH: XÓA HẲN KÊNH BRIDGE (destructive, dùng ngay trong kênh đó) ───
@bot.command(aliases=['delbridgechannel', 'xoabridge'])
async def xoakenhbridge(ctx):
    """Xóa hẳn kênh bridge hiện tại (gỡ toàn bộ liên kết + xóa luôn kênh). Chỉ Owner."""
    if ctx.author.id not in OWNER_IDS:
        return await ctx.reply("⛔ Không có quyền!", mention_author=False)

    links = get_links_for_bridge(ctx.channel.id)
    if not links:
        return await ctx.reply("⚠️ Kênh này không phải kênh bridge!", mention_author=False)

    for l in links:
        bridge_msg_map_col.delete_many({"target_channel_id": l["target_channel_id"]})
    bridge_links_col.delete_many({"bridge_channel_id": ctx.channel.id})
    bridge_webhook_cache.pop(ctx.channel.id, None)

    await ctx.send(embed=discord.Embed(description=f"✅ Đã hủy {len(links)} liên kết. Kênh này sẽ tự xóa sau 5 giây.", color=discord.Color.orange()))
...
# ── LỆNH: DANH SÁCH LIÊN KẾT (nhóm theo từng bridge) ───────────────────
@bot.command(aliases=['dslienket', 'listbridge'])
async def ds_lienket(ctx):
    """Xem toàn bộ liên kết đang hoạt động, nhóm theo từng kênh bridge. Chỉ Owner."""
    if ctx.author.id not in OWNER_IDS:
        return await ctx.reply("⛔ Không có quyền!", mention_author=False)
    ...
    if not links:
        return await ctx.reply(embed=discord.Embed(
            description="📡 Chưa có liên kết nào đang hoạt động.\nDùng `k lienket <channel_id>` để tạo!",
            color=discord.Color.light_grey()
        ), mention_author=False)

    grouped = {}
    for l in links:
        grouped.setdefault(l["bridge_channel_id"], []).append(l)

    lines = []
    for bridge_id, targets in grouped.items():
        bch = bot.get_channel(bridge_id)
        lines.append(f"**{bch.mention if bch else f'⚠️(kênh {bridge_id} đã xóa)'}** — {len(targets)} nguồn:")
        for t in targets:
            lines.append(f"   • #{t['target_channel_name']} ({t['target_guild_name']}) — `{t['target_channel_id']}`")

    embed = discord.Embed(
        title="📡 DANH SÁCH LIÊN KẾT ĐANG HOẠT ĐỘNG",
        description="\n".join(lines)[:4000],
        color=discord.Color.blue()
    )
    embed.set_footer(text=f"Tổng: {len(links)} liên kết trên {len(grouped)} kênh bridge")
    await ctx.reply(embed=embed, mention_author=False)
    # =====================================================================
